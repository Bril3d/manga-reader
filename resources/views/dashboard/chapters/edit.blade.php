...
<div class="flex flex-col gap-5">
    @include('dashboard.header', ['title' => __('Edit Manga :manga - Chapter :chapter', [
    'manga' => $chapter->manga->title,
    'chapter' => $chapter->chapter_number,
    ])])
    ...
    <form action="{{ route('dashboard.chapters.update', $chapter->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4" id="chapter-form">
        @csrf
        @method('PUT')

        <x-form.group>
            <x-form.select label="{{ __('Manga') }}" name="manga_id" selected="{{ old('manga_id', $chapter->manga->id) }}" :options="$mangas"></x-form.select>
            <x-form.input name="chapter_number" :value="old('chapter_number', $chapter->chapter_number)" placeholder="{{ __('Chapter Number') }}" />
        </x-form.group>

        <x-alert.info>{{ __('Keep the fields below empty to change only the manga and chapter number') }}</x-alert.info>
...
@vite(['themes/'. config('theme.active') .'/js/dashboard/chapter.js'])
@endsection
